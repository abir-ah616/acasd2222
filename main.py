            ...
            while True:
                ...
                if data2.hex().startswith('0500') and len(data2.hex()) > 1000:
                    try:
                        ...
                        message = f'Hi! I am a bot made by MR OPPY'
    ...
    while True:
        ...
        try:
            if action == "emote":
                ...
                # Send all emotes simultaneously using asyncio.gather
                async def send_single_emote(target_id):
                ...
                await asyncio.gather(*[send_single_emote(tid) for tid in player_ids])
                print(f"✓ Sent emote {emote_id} to {len(player_ids)} players simultaneously!")

            elif action == "emote_batch":
                assignments = command.get("assignments", [])
                
                # Send all assigned emotes simultaneously
                async def send_assigned_emote(assignment):
                ...
                await asyncio.gather(*[send_assigned_emote(a) for a in assignments])
                print(f"✓ Sent {len(assignments)} assigned emotes simultaneously!")
